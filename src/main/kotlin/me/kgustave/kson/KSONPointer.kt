...
class KSONPointer {
    companion object {
        @JvmStatic private val ENCODING = "UTF-8"
        @JvmStatic private val SEPARATOR_REGEX = Regex("/")
    ...
    class Builder: Appendable {
        ...
        override fun append(csq: CharSequence?): Builder {
        ...
        override fun append(c: Char) = if(c.isDigit()) append(c.toInt()) else append(c.toString())
        fun append(arrayIndex: Int) = append(arrayIndex.toString())

        operator fun div(csq: CharSequence?) = append(csq)
        operator fun div(index: Int) = append(index)
        operator fun get(index: Int) = append(index)

        operator fun String.div(csq: CharSequence?) = append(this).append(csq)
        operator fun Int.div(csq: CharSequence?) = append(this).append(csq)
        operator fun String.div(index: Int) = append(this).append(index)

        fun build() = KSONPointer(refTokens)
    }
}